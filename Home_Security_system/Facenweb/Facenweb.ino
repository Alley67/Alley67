...
const char* ssid = "Pixel_6721";
const char* password = "glory919";
 // Generate the beep tone
  int beepFrequency = 2000; // Adjust this value to change the frequency of the beep
  int beepDuration = 2000;  // Adjust this value to change the duration of the beep
...
void setup() {
 ...
 // setClock();                           // Set Time/date for TLS certificate validation
  ...
  mtmn_config = mtmn_init_config();     // Set  face recognition details (default values)
...
delay(500);
  ...
  delay(beepDuration);
 ledcWriteTone(0, 0); // Stop the tone by setting the duty cycle to 0
  delay(500);
   digitalWrite(ledPin, HIGH);
  delay(500);
  digitalWrite(ledPin, LOW);
        ...
        if (align_face(detected_face, image_matrix, aligned_face) == ESP_OK) {
          ...
          if (g_state == START_RECOGNITION  && (st_face_list.count > 0)) {
            ...
            if (face_recognized) {
              ...
              client.send("PERSON NOT RECOGNISED");
               digitalWrite(ledPin, HIGH);
  delay(500);
  digitalWrite(ledPin, LOW);
delay(500);
...
delay(500);
 ...
 ledcWriteTone(0, 0); // Stop the tone by setting the duty cycle to 0
  delay(500);
        ...
        if (g_state != START_DETECT) {
          client.send("NO PERSON DETECTED");
        }
      }
